...
!        if(ABS(user3)>1.5) then
!           if (ynew(j).lt.110.) then 
!              dep= 101.+(user3-10.)*(ynew(j)-110.)  
!           elseif (ynew(j).lt.118.) then 
!              dep= 101. + 1.5*(ynew(j)-108.)**2 -0.1*(ynew(j)-118.)**2 
!           elseif (ynew(j).lt.140.) then 
!              dep= 310. +30*(ynew(j)-120.) 
...
!             endif 
!          end do 
!          write(6,*) 'some prob in ini_topog' 
!          write(6,*) yc(j), jj 
!          stop 
!   101 end do 
...
!     Ddy = b(i) + 2*c(i) (x-x(i)) + 3* d(i) (x-x(i))^2                 
  ...
  !NF_NOWRITE tells netCDF we want read-only access to the file.
    ...
    print*, TRIM(dirin)//'input.nc'
  
  !Get the varid of the data variable, based on its name.
    retval = nf_inq_varid(ncid, 'D', varid)
  
  !Read the data.
    retval = nf_get_var_double(ncid, varid, D)

  !Get the varid of the data variable, based on its name.
    retval = nf_inq_varid(ncid, 'Ddx', varid)
  
  !Read the data.
    retval = nf_get_var_double(ncid, varid, Ddx)

  !Get the varid of the data variable, based on its name.
    retval = nf_inq_varid(ncid, 'Ddy', varid)
  
  !Read the data.
    retval = nf_get_var_double(ncid, varid, Ddy)

   print*,'MAX DEPTH = ', D(1,1), D(NI,NJ)
